...
FilePath: /codes/LLMRec/RLMRec/encoder/trainer/trainer.py
...
class Trainer(object):
    ...
    def train_epoch(self, model, epoch_idx):
        ...
        train_dataloader = self.data_handler.train_dataloader  # 训练数据
        train_dataloader.dataset.sample_negs()  # 负采样  搞了一个负采样[tensor[user...], tensor[pos...], tensor[neg...]
    ...
    def train(self, model):
        ...
        best_epoch = 0
        best_recall = -1e9
        self.create_optimizer(model)  # 这里是优化器
        train_config = configs['train']   # 参数
        ...
        # output test user embedding
        # self.out_entity_emb(model)


    @log_exceptions
    def out_entity_emb(self, model):
        print("生成test的embedding")
        model.eval()
        # 创建空的DataFrame
        result_df = pd.DataFrame()
        for ele, tem in enumerate(self.data_handler.test_dataloader):
            ...
            with torch.no_grad():
                ...
                df_out_entity_emb = model.output_entity_embedding(batch_data)
                result_df = pd.concat([result_df, df_out_entity_emb], ignore_index=True)

        model_name = configs['model']['name']
        save_dir_path = './encoder/test_emb/{}'.format(model_name)
        if not os.path.exists(save_dir_path):
            os.makedirs(save_dir_path)

        result_df.to_csv(save_dir_path + '/{}_test_user_emb.txt'.format(configs['data']['name']), sep='\t', index=None)
        # result_df.to_csv(save_dir_path + '/{}_DALR_test_user_emb.txt'.format(configs['data']['name']), sep='\t', index=None)
