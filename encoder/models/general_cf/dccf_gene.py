...
class DCCF_gene(BaseModel):
    def __init__(self, data_handler):
        ...
        self.user_update_embs = torch.tensor(configs['user_update_embs']).float().cuda()
        self.item_update_embs = torch.tensor(configs['item_update_embs']).float().cuda()
        # mlp_dw
    ...
    def cal_loss(self, batch_data):
        ...
        loss = bpr_loss + reg_loss + cl_loss + recon_loss + align_loss
        # loss = bpr_loss + reg_loss + align_loss
        losses = {'bpr_loss': bpr_loss, 'reg_loss': reg_loss, 'cl_loss': cl_loss, 'recon_loss': recon_loss, 'align_loss': align_loss}
        return loss, losses
